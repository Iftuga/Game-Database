...
def display():
    ...
    class scrollFrame(tk.Frame):
        def __init__(self, parent, *args, **kw):
            ...
            #vscrollbar.place( x = 725, y = 75, whi)
...
#Поле = {"Название игры":0, "Жанр":1, "Платформа":2, "Год выпуска":3, "Цена":4, "Разработчик":5, "Издатель":6}
        def buttSort(self):
            ...
            while ( i < len(self.base)):
                ...
                """
                self.pP[i].place(x = 125, y = ( i + 1 ) * 25, width = 100, height = 25 )
                self.pG[i].delete(0,1999)
                self.pG[i].insert(0,self.base[i][1])
                self.pG[i].place(x = 225, y = ( i + 1 ) * 25, width = 50, height = 25 )
                self.pY[i].delete(0,1999)
                self.pY[i].insert(0,self.base[i][3])
                self.pY[i].place(x = 275, y = ( i + 1 ) * 25, width = 100, height = 25 )
                self.pD[i].delete(0,1999)
                self.pD[i].insert(0,self.base[i][5])
                self.pD[i].place(x = 375, y = ( i + 1 ) * 25, width = 100, height = 25 )
                self.pPu[i].delete(0,1999)
                self.pPu[i].insert(0,self.base[i][6])
                self.pPu[i].place(x = 475, y = ( i + 1 ) * 25, width = 100, height = 25 )
                self.pPr[i].delete(0,1999)
                self.pPr[i].insert(0,self.base[i][4])
                self.pPr[i].place(x = 575, y = ( i + 1 ) * 25, width = 50, height = 25 )
                """
                i = i + 1
            #self.pNG.pack(side = tk.LEFT)
